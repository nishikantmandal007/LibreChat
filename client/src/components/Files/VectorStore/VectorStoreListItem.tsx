...
export default function VectorStoreListItem({
  ...
  return (
    <div
      ...
      className="glass-surface w-100 mt-2 flex cursor-pointer flex-row justify-around rounded-xl p-4 transition duration-300 ease-in-out hover:bg-[var(--glass-bg-hover)]"
    >
      <div className="flex w-1/2 flex-col justify-around align-middle">
        <strong>{vectorStore.name}</strong>
        <p className="text-sm text-[var(--glass-text-secondary)]">{vectorStore.object}</p>
      </div>
      ...
      <div className="flex w-1/6 flex-col justify-around sm:flex-row">
        <Button className="m-0 w-full content-center bg-transparent p-0 text-[var(--glass-text-secondary)] hover:bg-slate-200 sm:w-min">
