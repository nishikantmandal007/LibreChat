...
export default function FileListItem2({
  ...
  return (
    <div
      ...
      className="glass-surface w-100 mt-2 flex h-fit cursor-pointer flex-row rounded-xl p-4 transition duration-300 ease-in-out hover:bg-[var(--glass-bg-hover)]"
    >
      <div className="flex w-10/12 flex-col justify-around md:flex-row">
        ...
        <div className="flex w-fit flex-row flex-wrap text-[var(--glass-text-secondary)] md:w-3/5">
          {attachedVectorStores.map((vectorStore, index) => {
            if (index === 4) {
              return (
                <span
                  key={index}
                  className="ml-2 mt-1 flex flex-row items-center rounded-full bg-[var(--glass-bg-subtle)] px-2 text-xs"
                >
      ...
      <div className="mr-0 flex w-2/12 flex-col items-center justify-evenly sm:mr-4 md:flex-row">
        <Button className="w-min content-center bg-transparent text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-hover)]">
          <DotsIcon className="text-grey-100" />
        </Button>
        <Button
          className="w-min bg-transparent text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-hover)]"
